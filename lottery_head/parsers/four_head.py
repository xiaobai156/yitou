...
from .common import canonical_text, contains_text, has_open_marker, period_matches, period_number, unique_extracted_records


def parse_four_combo_after_marker_records(compact: str, field: str, target_period: str = "") -> list[dict[str, str]]:
    match_marker = canonical_text(field)
    match_compact = canonical_text(compact)
    marker_index = match_compact.find(match_marker)
    source = compact[marker_index:] if marker_index >= 0 else compact
    records = []
    all_heads = {"0", "1", "2", "3", "4"}
...
def parse_anchored_four_combo_missing_head_records(
    ...
    if anchor_index < 0:
        return []
    source = compact[anchor_index + len(anchor_marker):]
...
def parse_manager_anchored_four_combo_missing_head_records(
    ...
    for block in re.finditer(r"(\d{2,3}期)(.*?)(?=\d{2,3}期|$)", compact):
        ...
        if not period_matches(period, target_period):
            continue
        body = block.group(2)
        if not contains_text(body, anchor_marker) or not contains_text(body, field) or not has_open_marker(body):
            continue
        values = extract_four_combo_values(body)
        if len(values) != 4 or len(set(values)) != 4:
            continue
...
def parse_topic_title_anchored_four_combo_missing_head_records(
    ...
    return [record for record in current_records if period_matches(record["period"], target_period)]
...
def extract_four_combo_values(text: str) -> list[str]:
    ...
    for bracket_match in re.finditer(r"[【\[〖\(（《〈]([^】\]〗\)）》〉]{1,30})[】\]〗\)）》〉]", text):
